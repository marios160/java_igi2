...
public class AutoBan {
    ...
    public static void ban(String ip) {
        try {
            ...
            switch (AutoBan.sysi) {
                case 1:
                    cmd = odkoduj(windowsban) + ip + "/32";
                    break;
                case 2:
                    cmd = odkoduj(linuxban1) + ip + odkoduj(linuxban2);
                    break;
                default:
                    return;
            }
            ...
            Console.setmessage(odkoduj(fcrasher) + ip);
